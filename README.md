...
### V4.7 数独题库验证与体验优化 🔍 (Latest)
1.  **题目库重构**：
    *   **17提示极限数独**：全面替换了极限难度（Extreme）的题目库。之前的题库中部分题目提示数多于17或存在多解，现已更新为 **8个经过严格验证的17提示数独**（包含经典题目的旋转变体），确保唯一解且满足理论极限。
    *   移除了不可行的伪极限题目。
2.  **视觉体验优化**：
    *   **笔记清晰化**：大幅增大了数独笔记模式下的数字字体大小（`0.75rem`），并加深了颜色，使其在移动端阅读更加清晰舒适。
    *   **填入数字加粗**：将用户填入的数字样式修改为 **粗体 (bold)**，使其与初始数字的视觉权重更加平衡，同时更易于辨识。
3.  **游戏难度平衡**：
    *   **容错率统一**：为了降低高难度下的挫败感，现在 **地狱 (Hell)** 和 **极限 (Extreme)** 模式同样允许 **3次错误**（原为2次和1次）。
